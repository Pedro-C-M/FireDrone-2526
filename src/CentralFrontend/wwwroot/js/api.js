...
// === L�GICA DE ACTUALIZACI�N Y EDICI�N ===
...
// === FUNCI�N DE VISUALIZACI�N CORREGIDA ===
function _displayFlightPlans(data) {
    const tBody = document.getElementById('flightplans_tbody');

    // 1. Protecci�n inicial: Si no hay tabla, no hacemos nada
    if (!tBody) return;

    tBody.innerHTML = '';
    _displayCount(data.length);

    // 2. Protecci�n de template: Aseguramos que el template existe
    ...
    data.forEach(flightplan => {
        ...
        // Funci�n auxiliar: Intenta buscar el bot�n y a�adir el evento.
